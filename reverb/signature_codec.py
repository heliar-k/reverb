...
def _decode(proto: reverb_tensor_pb2.SignatureProto) -> Any:
  ...
  if kind == 'named_tuple_value':
    ...
    fields = list(nt.keys) if nt.keys else [
        f'_{i}' for i in range(len(nt.values))
    ]
    cls = collections.namedtuple(nt.name or 'SignatureNamedTuple', fields,
                                 rename=True)
