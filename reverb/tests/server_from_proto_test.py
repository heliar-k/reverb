...
class ServerFromProtoTest(parameterized.TestCase):
    ...
    def test_selector_from_proto(self, selector_proto_field, expected_selector):
    ...
    def test_heap_selector_from_proto(self, is_min_heap, expected_selector_builder):
        selector_proto = schema_pb2.KeyDistributionOptions()
    ...
    def test_rate_limiter_from_proto(self):
        ...
        rate_limiter = server_from_proto.rate_limiter_from_proto(rate_limiter_proto)
        self.assertIsInstance(rate_limiter, reverb.rate_limiters.RateLimiter)
    ...
    def test_table_from_proto(self):
        ...
        self.assertEqual(table_info.max_times_sampled, table_proto.max_times_sampled)
        # The rate limiter objects do not have quite the same structure.
        self.assertEqual(
            table_info.rate_limiter_info.max_diff, table_proto.rate_limiter.max_diff
        )
        self.assertEqual(
            table_info.rate_limiter_info.min_diff, table_proto.rate_limiter.min_diff
        )
        self.assertEqual(
            table_info.rate_limiter_info.samples_per_insert,
        ...
        self.assertEqual(table_info.sampler_options.lifo, table_proto.sampler.lifo)
        self.assertEqual(table_info.remover_options.fifo, table_proto.remover.fifo)
