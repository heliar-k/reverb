...
from reverb import errors, pybind, trajectory_writer
...
class TrajectoryWriterTest(parameterized.TestCase):
    def setUp(self):
        ...
        def _mock_create_item(
            unused_table, unused_priority, refs_per_col, squeeze_per_col
        ):
            item = []
    ...
    def test_configure_seen_column(self):
        ...
        self.cpp_writer_mock.ConfigureChunker.assert_called_with(
            0, pybind.ConstantChunkerOptions(num_keep_alive_refs=2, max_chunk_length=1)
        )
    ...
    def test_configure_unseen_column(self):
        ...
        self.cpp_writer_mock.ConfigureChunker.assert_called_with(
            3, pybind.ConstantChunkerOptions(num_keep_alive_refs=2, max_chunk_length=1)
        )
    ...
    def test_configure_validates_params(
        self, num_keep_alive_refs: int, max_chunk_length: Optional[int], valid: bool
    ):
        if valid:
    ...
    def test_exit_does_not_flush_on_reverb_error(self):
        ...
        with mock.patch.object(self.writer, "flush") as flush_mock:
            with self.assertRaises(errors.ReverbError), self.writer:
...
class TrajectoryColumnTest(parameterized.TestCase):
    ...
    def test_shape_squeezed(self):
        ...
        column = trajectory_writer.TrajectoryColumn(
            [FakeWeakCellRef(data)], squeeze=True
        )
