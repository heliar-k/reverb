...
class TestSampleToInsertRatio(parameterized.TestCase):
    ...
    def test_validates_single_number_error_buffer(
        self, samples_per_insert, error_buffer, want
    ):
        if want:
    ...
    def test_validates_explicit_range_error_buffer(
        self, min_size_to_sample, samples_per_insert, error_buffer, want
    ):
        if want:
            with self.assertRaises(want):
...
class TestRateLimiterBase(absltest.TestCase):
    ...
    def test_subclasses(self):
        ...
        self.assertIsInstance(
            rate_limiters.SampleToInsertRatio(1.0, 10, 5), rate_limiters.RateLimiter
        )
