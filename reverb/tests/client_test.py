...
class ClientTest(absltest.TestCase):
    ...
    def test_sample_sets_priority(self):
        ...
        self.assertSequenceAlmostEqual(
            [1000.0] * len(original_priorities), original_priorities.values()
        )
        expected_priorities = {
    ...
    def test_sample_trajectory_without_signature(self):
        ...
        sample = next(
            self.client.sample(
                SIMPLE_QUEUE_NAME, emit_timesteps=False, unpack_as_table_signature=True
            )
        )
    ...
    def test_sample_trajectory_written_with_insert(self):
        ...
        sample = next(self.client.sample(SIMPLE_QUEUE_NAME, emit_timesteps=False))
...
class ClientBoundaryTest(ClientTest):
    ...
    def test_server_address_property(self):
        self.assertEqual(self.client.server_address, f"localhost:{self.server.port}")
...
class LocalClientBoundaryTest(absltest.TestCase):
    ...
    def test_trajectory_writer_invalid_refs_raises(self):
        with self.assertRaises(ValueError):
            self.client.trajectory_writer(num_keep_alive_refs=0)

    def test_trajectory_writer_negative_refs_raises(self):
        with self.assertRaises(ValueError):
            self.client.trajectory_writer(num_keep_alive_refs=-1)

    def test_trajectory_writer_max_chunk_length(self):
        w = self.client.trajectory_writer(num_keep_alive_refs=2, max_chunk_length=1)
