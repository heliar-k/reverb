...
from reverb import pybind, replay_sample


class SampleInfoTest(absltest.TestCase):
    def test_has_the_correct_number_of_fields(self):
        self.assertLen(
            replay_sample.SampleInfo._fields, pybind.Sampler.NUM_INFO_TENSORS
        )
    ...
    def test_construct_with_arrays(self):
        ...
        info = replay_sample.SampleInfo(
            key=keys, probability=probs, table_size=3, priority=1.0, times_sampled=1
        )
