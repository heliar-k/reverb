...
class Server:
    ...
    def __del__(self):
        ...
        # C1: stop() handles ShmServer cleanup (Stop + nulling). Guard with hasattr
        # so __del__ does not crash if __init__ raised before state was set.
        if (
            hasattr(self, "_server")
            and self._server is not None
            or hasattr(self, "_shm_server")
            and self._shm_server is not None
        ):
            self.stop()
