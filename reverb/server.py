...
class Server:
    ...
    def __init__(
        ...
        Args:
          ...
          shm: If True, also start a POSIX shared-memory transport (decision C1): a
        ...
        if duplicates:
            raise ValueError(
                "Multiple items in tables have the same name: {}".format(
                    ", ".join(duplicates)
                )
            )
        ...
        self._shm_socket_path = None
        self._shm_socket_owned = False
        ...
        if shm:
            ...
            # table (tables[0]); multi-table is deferred (// ponytail:). SHM is an
            # additional transport layered on the owned Table, independent of
    ...
    @property
    def shm_socket_path(self) -> Optional[str]:
        """udsocket path clients connect `ShmClient` to (decision C1).

        Only available when the server was constructed with `shm=True`. The path
        is unlinked on `stop()`.
        """
        return self._shm_socket_path

    def __del__(self):
        """Stop server, ShmServer, and free up the port if reserved via portpicker."""
        # C1: tear down the SHM transport too. Guard with hasattr so __del__ does
        # not crash during interpreter shutdown if pybind objects are gone.
        if hasattr(self, "_shm_server") and self._shm_server is not None:
            try:
    ...
    def __repr__(self) -> str:
        ...
        if self._shm_server is not None:
            return f"ShmServer(socket_path={self._shm_socket_path})"
    ...
    def stop(self):
        ...
        Also stops the SHM transport (`ShmServer`) if one was started (C1): joins
        the dispatch thread, cleans up clients, and unlinks the udsocket.
        """
        # C1: stop the SHM server first (it holds a borrowed ref to the Table).
        if self._shm_server is not None:
            self._shm_server.Stop()
            self._shm_server = None
        if self._server is not None:
