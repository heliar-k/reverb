...
class TrajectoryWriter:
    ...
    def append(self, data: Any, *, partial_step: bool = False):
        ...
        try:
            ...
            self._update_structure(
                _tree_union(self._structure, tree.map_structure(lambda x: None, data))
            )
        ...
        # None.
        for column, data_reference in zip(
            self._column_history, flat_column_data_references
        ):
    ...
    def _maybe_create_column(self, path, column_index):
        ...
        if path in self._path_to_column_config:
            self._writer.ConfigureChunker(
                column_index, self._path_to_column_config[path]
            )
        return column_index
    ...
    def _update_structure(self, new_structure: Any):
        ...
        # `_structure` itself.
        self._structure = tree.unflatten_as(
            new_structure,
            [
...
class _ColumnHistory:
    ...
    def __init__(
        ...
        self._data_references: List[Optional[pybind.WeakCellRef]] = [None] * min(
            buffer_size, history_padding
        )
...
class TrajectoryColumn:
    ...
    def __init__(
        ...
        *,
        ...
        if any(ref is None for ref in data_references):
            raise ValueError(
                "TrajectoryColumns cannot contain any None data "
    ...
    def __getitem__(self, val) -> "TrajectoryColumn":
        if isinstance(val, int):
            ...
            raise TypeError(
                f"TrajectoryColumn indices must be integers or slices, not {type(val)}"
            )
...
def _tree_union(a, b):
    ...
    if isinstance(a, (List, Tuple)):
        return type(a)(_tree_union(aa, bb) for aa, bb in itertools.zip_longest(a, b))
