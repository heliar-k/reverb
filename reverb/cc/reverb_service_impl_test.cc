...
// ponytail: 原 TF StructuredValue 已迁移为 reverb_tensor.SignatureProto。
// 构造与生产代码(Table::signature_)同型的 scalar Int32 tensor spec。
::reverb::tensor::SignatureProto MakeSignature() {
  ::reverb::tensor::SignatureProto signature;
  auto* tensor_spec = signature.mutable_tensor_spec();
  tensor_spec->set_name("item0");
  tensor_spec->set_dtype(::reverb::tensor::DT_INT32);
  return signature;
}
...
TEST(ReverbServiceImplTest, CheckpointAndLoadFromCheckpoint) {
  // ponytail: 原 TF Env::CreateUniqueFileName 改用 mkdtemp 生成唯一临时目录
  // (同 in_process_client_test 的 MakeCkptRoot 模式),避免跨测试 checkpoint 路径冲突。
  auto base = std::filesystem::temp_directory_path() / "reverb_svc_ckptXXXXXX";
