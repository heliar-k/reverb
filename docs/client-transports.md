...
| `checkpoint` / 恢复 | ✅ | ✅（`Server(in_process=True)` 构造时自动 `LoadLatest`） | ✅（走 insert 流；恢复经共享 Table 的 `LoadLatest` 搭车 gRPC/InProcess） |
...
**`trajectory_writer` 的 signature 校验已生效**：`NewTrajectoryWriter` 调
...
开关）。无签名的表跳过校验。注意：`NewTrajectoryWriter` 构造时获取签名并缓存；
会话中途 `Table.replace` 改签名后，旧 writer 仍持旧签名，需重连刷新。
...
initializer）时，`ShmClient` 也可 pickle——前提是 worker 与 server 同机（SHM
本来就只跨进程不跨机）。
...
### 5.7 `ShmClient.checkpoint()` 走 insert 流

`checkpoint()` 与 `mutate_priorities`/`reset` 同走 INSERT 流（`insert_c2s`/
`insert_s2c`），在 `ShmConnection::insert_flow_mu` 下串行化 send→read-ACK。
服务端 `HandleCheckpoint` 调注入的 `checkpointer_->Save` 落盘所有表，经
`CHECKPOINT_RESP` 返回路径。恢复路径无新代码：`ShmServer` 与 gRPC/InProcess
共享同一批 `Table` 对象，构造时 `ReverbServiceImpl::Initialize` 或
`InProcessClient::LoadLatest` 已对它们 `LoadLatest`，SHM 自动看到恢复态。
...
三者 pybind 命名：gRPC `Client` 绑 PascalCase；`InProcessClient` 同时绑 snake_case
与 PascalCase 双名别名（见 [numpy-embed-design.md §3.3](numpy-embed-design.md)）；
`ShmClient` 的 `NewSampler` 用 PascalCase 与 `_BaseClient` 统一路径对齐。
